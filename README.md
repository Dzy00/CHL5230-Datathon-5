...
## Introduction
- This project investigate whether recurrent neural network (RNN) and long short-term memory (LSTM) models can recognize human activities using wearable sensor data, and examines which types of motion signals (sedentary vs dynamic) contribute more on activity recognition.
...
- Models & Architecture: 
  ...
  - Long Short-Term Memory (LSTM): 1 LSTM layer, 1 dropout layer, 2 fully connected linear layers. Activation function: ReLU. Loss: cross-entropy. Optimizer: Adam.
...
- Results:
  - RNN: training accuracy plateaued at 72%-76%, and validation accuracy plateaued at 69%-71%. Both training and validation accuracy decreased significantly after removing Idle class.
  - LSTM: training accuracy ranged from 72-78%, and validation accuracy ranged from 68-72%. Performance dropped significantly after removing Idle class.
- Key takeaways: LSTM outperformed RNN with modest improvements. Both models primarily learned to predict idle class driven by class imbalance. Regularization had limited performance improvements. Train accuracy was consistently higher than validation accuracy indicating imperfect generalization. 
